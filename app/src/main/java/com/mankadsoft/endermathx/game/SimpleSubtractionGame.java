...
public class SimpleSubtractionGame implements MathGame {

    private static final int MAX_VAL = 30;
    private static final int GAME_DURATION = 60;
    public static final String GAME_NAME = "Simple Subtraction";
    ...
    public Problem generateProblem() {
        ...
        if(hardChance < RECHALLENGE_RATE && hardestProblems.size()>0) {
            ...
            int numA = getRandomNumberInRange(1, 19);
            int numB = getRandomNumberInRange(1, 10);

            Log.d("mankad", "Presenting New Problem.");
            return new Problem(Long.toString(java.util.Calendar.getInstance().getTimeInMillis()),
                    Integer.toString(Math.max(numA, numB)),
                    Integer.toString(Math.min(numA, numB)),
                    "-");
        }
    }
    ...
    public Result checkAnswer(Problem problem, String attemptedAnswer) {
        try {
            ...
            if (valA - valB == answer) {
                return new Result(problem, attemptedAnswer, Result.CORRECT, 3);
            } else {
