...
public class SimpleAdditionGame implements MathGame {

    private static final int MAX_VAL = 9;
    private static final int GAME_DURATION = 60;
    public static final String GAME_NAME = "Simple Addition";
    ...
    public Problem generateProblem() {
        ...
        if(hardChance < RECHALLENGE_RATE && hardestProblems.size()>0) {
            ...
            return new Problem(Long.toString(java.util.Calendar.getInstance().getTimeInMillis()),
                    Integer.toString(getRandomNumberInRange(0, MAX_VAL)),
                    Integer.toString(getRandomNumberInRange(0, MAX_VAL)),
                    "+");
        }
    }
    ...
    public Result checkAnswer(Problem problem, String attemptedAnswer) {
        try {
            ...
            if (valA + valB == answer) {
                return new Result(problem, attemptedAnswer, Result.CORRECT, 1);
            } else {
